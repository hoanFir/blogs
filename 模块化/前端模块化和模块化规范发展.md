...
CommonJS 是以同步方式加载模块的，在Node的执行机制当中，是启动时加载模块，而在代码执行当中只是去查找并使用模块。但是对于浏览器，这种同步机制在浏览器端用户体验是很不友好的，因为大量同步请求会导致阻塞，响应效率低下。
...
同期，AMDJS 推出 Require.js 库，是一个强大的实现了 AMD 模块化规范的模块加载器。
