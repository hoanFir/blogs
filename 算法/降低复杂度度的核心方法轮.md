...
设计合理数据结构，完成时间复杂度向空间复杂度的转化，因为时间更为昂贵
...
public void fn() {
  ...
  int time_max = 0; //保存当前循环次数最多的数值i
  int time_tmp = 0; //保存每次循环的arr[i]在数组中重复的个数
  
  int val_max = 1; //保存当前循环次数最多的数值i对应的arr[i]
...
public void fn() {
  ...
  Map<Integer, Integer> d = new HashMap<>(); //创建一个Map存储数组每个位置值出现次数
